...
//页头
    ...
    $("input[type=radio]").click(function () {
        if ($(this).prop("checked")==true){
            $("#name").html($(this).parent().next().html());
            $("#phone").html($(this).parent().next().next().html());
            $("#addr").html($(this).parent().next().next().next().html());
        }else{
            $("#name").html(a_g[0][0].userName);
            $("#phone").html(a_g[0][0].phoneName);
            $("#addr").html(a_g[0][0].userAddress1);
        }
    })

});

var orderNum=parseInt(Math.random()*99999999);
window.onload=function () {
    var a=0;


    $('#orderN').html(orderNum);
    for (var i=0;i<$(".mon").length;i++){
        a+=parseInt($(".mon")[i].innerText);
    }
    $("#monLi").html(a+".00 元");
};
...
$('#buy').click(function () {
    var mydate=new Date();
    var datefor=mydate.toLocaleString().replace(/\//g,"-");
    datefor=datefor.substring(0,10);
    var dataArr=[];
    dataArr.push(datefor);
    dataArr.push(orderNum);
    dataArr.push($("#name").html());
    dataArr.push($("#phone").html());
    dataArr.push($("#addr").html());
